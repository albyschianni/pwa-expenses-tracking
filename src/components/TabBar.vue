<template>
  <nav class="fixed bottom-0 left-0 w-full h-16 bg-gray-900 z-50">
    <div class="flex items-center justify-evenly h-full">
      <!-- Home tab -->
      <button
        @click="$emit('navigate', 'home')"
        class="flex flex-col items-center gap-1 transition-colors"
        :class="activeTab === 'home' ? 'text-teal-400' : 'text-gray-400'"
      >
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
        </svg>
        <span class="text-xs">Home</span>
      </button>

      <!-- Graphic tab -->
      <button
        @click="$emit('navigate', 'graphic')"
        class="flex flex-col items-center gap-1 transition-colors"
        :class="activeTab === 'graphic' ? 'text-teal-400' : 'text-gray-400'"
      >
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6a7.5 7.5 0 1 0 7.5 7.5h-7.5V6Z" />
          <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0 0 13.5 3v7.5Z" />
        </svg>
        <span class="text-xs">Graphic</span>
      </button>

      <!-- Center FAB with spacer -->
      <div class="relative w-14">
        <button
          @click="$emit('fab-click')"
          class="absolute left-1/2 -translate-x-1/2 -top-12 w-14 h-14 bg-teal-400 rounded-full flex items-center justify-center shadow-lg shadow-teal-400/30 active:scale-95 transition-transform"
        >
          <svg class="w-7 h-7 text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
          </svg>
        </button>
      </div>

      <!-- TODO tab -->
      <button
        @click="$emit('navigate', 'todo')"
        class="flex flex-col items-center gap-1 transition-colors"
        :class="activeTab === 'todo' ? 'text-teal-400' : 'text-gray-400'"
      >
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 5.25h.008v.008H12v-.008Z" />
        </svg>
        <span class="text-xs">TODO</span>
      </button>

      <!-- Settings tab -->
      <button
        @click="$emit('navigate', 'settings')"
        class="flex flex-col items-center gap-1 transition-colors"
        :class="activeTab === 'settings' ? 'text-teal-400' : 'text-gray-400'"
      >
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
        <span class="text-xs">Settings</span>
      </button>
    </div>
  </nav>
</template>

<script setup>
defineProps({
  activeTab: {
    type: String,
    default: 'home'
  }
})

defineEmits(['fab-click', 'navigate'])
</script>
