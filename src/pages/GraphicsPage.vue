        ...
        <div class="bg-gray-800 rounded-2xl p-4">
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <button
                class="w-full flex items-center justify-between active:opacity-70 transition-opacity"
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-gray-500 text-xs">({{ getExpensesForCategory(cat.categoryId).length }})</span>
                </div>
                <div class="flex items-center gap-2">
                  <span
                  ...
                  <svg
                    ...
                    :class="expandedCategory === cat.categoryId ? 'rotate-180' : ''"
              ...
              <Transition name="expand">
                <div
                  v-if="expandedCategory === cat.categoryId"
                  class="mt-1 rounded-xl overflow-hidden border border-gray-700"
                >
                  <div
                    v-for="tx in getExpensesForCategory(cat.categoryId)"
                    :key="tx.id"
                    class="flex items-center justify-between px-3 py-2.5 border-b border-gray-700/60 last:border-0"
                  >
                    <div class="flex-1 min-w-0 mr-3">
                      <p class="text-gray-200 text-xs truncate">{{ tx.description || 'Pagamento prepagata' }}</p>
                      <p class="text-gray-500 text-[11px] mt-0.5">{{ formatDate(tx.date) }}</p>
                    </div>
                    <span
                      class="text-xs font-semibold shrink-0"
                      :class="chartType === 'income' ? 'text-emerald-400' : 'text-gray-300'"
                    >
                      {{ chartType === 'income' ? '+' : '-' }}{{ formatCurrency(tx.amount) }}
                    </span>
                  </div>
                </div>
              </Transition>
...
function getExpensesForCategory(categoryId: string) {
  return filteredExpenses.value.filter(e => (e.category || (e as any).category_id) === categoryId)
}
